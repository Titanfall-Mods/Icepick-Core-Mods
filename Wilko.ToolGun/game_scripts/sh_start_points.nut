...
array<StartPointCSV> function GetStartPointsForMap( string map )
{
	...
	{
		StartPointCSV newStart;
		newStart.name = map;
		newStart.loadScreenIndex = 0;
		newStart.hasDetente = false;
		newStart.spLog = "Missing start points for level " + map + ".";
		newStart.spLogTitle = "Missing Start Points";
		newStart.isLeft = false;
		file.startPointStrings[ map ] <- [ newStart ];
	}
